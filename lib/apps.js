...
const DESKTOP_ICON_DEFAULT_FRAC = {
  webasset_x_bock: { xp: 0.1, yp: 0.28 },
  webasset_logo: { xp: 0.25, yp: 0.25 },
  webasset_threads: { xp: 0.17, yp: 0.38 },
  webasset_clay: { xp: 0.11, yp: 0.55 },
  webasset_mm_series: { xp: 0.27, yp: 0.48 },
  webasset_kerze: { xp: 0.2, yp: 0.65 },
  webasset_step: { xp: 0.11, yp: 0.8 },
  webasset_crashbox: { xp: 0.34, yp: 0.75 },
  webasset_grillz: { xp: 0.39, yp: 0.51 },
  webasset_lsradio: { xp: 0.42, yp: 0.68 },
  notes: { xp: 0.77, yp: 0.45 },
  media: { xp: 0.74, yp: 0.68 },
};

export function getDefaultDesktopIconPositions() {
  const positions = { ...DESKTOP_ICON_DEFAULT_FRAC };
  ASSET_DESKTOP_ICONS.forEach((item) => {
    if (!positions[item.appId]) {
      positions[item.appId] = { xp: 0.15, yp: 0.35 };
    }
  });
