...
export function isSlideVideoFile(name) {
  return /\.(mp4|webm|mov|m4v)$/i.test(name);
}

/** Bilder oder Videos für die Desktop-Slideshow (Finder-Reihenfolge). */
export function isSlideshowMediaFile(name) {
  return isSlideImageFile(name) || isSlideVideoFile(name);
}
...
/**
 * Wie {@link listSlideImageFiles}, aber inkl. Videoformate für die Slideshow.
 * @param {string} dir
 * @param {string} [basePath="/web"]
 * @param {{ restrictTo?: string[] }} [options]
 * @returns {string[]}
 */
export function listSlideshowMediaFiles(dir, basePath = "/web", options) {
  const entry = webAssetManifest.find((x) => x.dir === dir);
  if (!entry?.files?.length) return [];
  const tree = buildAssetFileTree(entry.files);
  const rows = collectAssetTreeFlatRows(tree, dir, basePath, new Set());
  const finderMedia = rows
    .filter((r) => r.kind === "file" && isSlideshowMediaFile(r.fullPath))
    .map((r) => r.fullPath);

  const restrictTo = options?.restrictTo;
  if (restrictTo === undefined) return finderMedia;
  if (restrictTo.length === 0) return [];

  const allowed = new Set(finderMedia);
  return restrictTo.filter((f) => allowed.has(f));
}
